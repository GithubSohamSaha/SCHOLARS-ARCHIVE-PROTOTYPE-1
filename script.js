...
const users = [
	...
	{ id: 3, username: "manager1", password: "pass123", role: "librarian", name: "Rwita Pan", email: "sego@university.edu" },
...
// --- Shared storage (for multi-tab sync) ---
const STORAGE_KEYS = {
	books: 'lms_books',
	borrowRecords: 'lms_borrowRecords',
	fineRecords: 'lms_fineRecords',
	users: 'lms_users'
};
...
function saveDataToStorage() {
	...
	try {
		...
		localStorage.setItem(STORAGE_KEYS.borrowRecords, JSON.stringify(borrowRecords));
		localStorage.setItem(STORAGE_KEYS.fineRecords, JSON.stringify(fineRecords));
...
function loadDataFromStorage() {
	...
	try {
		...
		const storedBorrows = localStorage.getItem(STORAGE_KEYS.borrowRecords);
		const storedFines = localStorage.getItem(STORAGE_KEYS.fineRecords);
		...
		if (storedBorrows) {
			...
			borrowRecords.push(...parsed);
		}
		if (storedFines) {
			const parsed = JSON.parse(storedFines);
			fineRecords.length = 0;
			fineRecords.push(...parsed);
		}
		if (storedUsers) {
...
(function initStorage() {
	if (typeof localStorage === 'undefined') return;
	try {
		const alreadyInit = localStorage.getItem('lms_initialized');
		if (alreadyInit) {
			loadDataFromStorage();
		} else {
			// first load: persist the in-memory defaults
			saveDataToStorage();
...
function saveSession() {
    ...
    try {
        localStorage.setItem('lms_currentUserId', String(currentUser.id));
    } catch (e) {}
}
...
function clearSession() {
    if (typeof localStorage === 'undefined') return;
    try {
        localStorage.removeItem('lms_currentUserId');
    } catch (e) {}
}
...
function showLoginSection() {
    ...
    if (navLogout) navLogout.style.display = currentUser ? 'block' : 'none';
}
...
if (navLogin)  navLogin.addEventListener('click',   (e) => { e.preventDefault(); showLoginSection(); });
if (navSignup) navSignup.addEventListener('click',  (e) => { e.preventDefault(); showSignupSection(); });
if (navHome)   navHome.addEventListener('click',    (e) => { e.preventDefault(); showHome(); });
if (navSupport)navSupport.addEventListener('click', (e) => { e.preventDefault(); showSupport(); });
...
// Restore session on load, if any
(function restoreSession() {
    if (typeof localStorage === 'undefined') return;
    try {
        const id = localStorage.getItem('lms_currentUserId');
        if (!id) return;
...
// If no active session, show the Home page by default
if (!currentUser) {
    showHome();
}
...
function handleSignup(e) {
    ...
    const existing = users.find(u => u.username.trim().toLowerCase() === usernameLower);
    if (existing) {
...
function setupDashboard() {
	...
	dashboardTitle.textContent = `${roleLabel} Dashboard`;
...
// Manager responsibilities moved under Librarian role
function showCollectReturns() {
	dashboardContent.innerHTML = `<div class='card'><h3>Collect Returns</h3><p>Feature to collect book returns across all users (to be implemented).</p></div>`;
}

function showStudentAccounts() {
	dashboardContent.innerHTML = `<div class='card'><h3>User Accounts</h3><p>Feature to view and manage user borrowing accounts (to be implemented).</p></div>`;
}
...
function showLibrarianDashboard() {
	dashboardContent.innerHTML = `
		<div class="card">
			...
			<p>Welcome, ${currentUser.name}. You oversee the whole Scholar's Archive system.</p>
        ...
        if (studentFines.length === 0) {
            ...
            receiptDiv.innerHTML = `
                <div class="card">
                    ...
                    <p><strong>User:</strong> ${student.name}</p>
...
// Cross-tab sync: when another tab updates storage, refresh data here
window.addEventListener('storage', function(e) {
	if (!e.key || ![STORAGE_KEYS.books, STORAGE_KEYS.borrowRecords, STORAGE_KEYS.fineRecords, STORAGE_KEYS.users].includes(e.key)) {
		return;
	}
	loadDataFromStorage();
	if (currentUser) {
		// Rebuild the current user's dashboard so new fines/borrows are visible
		setupDashboard();
	} else {
		// If no one is logged in, ensure the public view is consistent
		showHome();
	}
});
...
// Helper to jump from Support/FAQ directly to the login screen with a given role pre-selected
window.goToRoleLogin = function(role) {
    showLoginSection();
